...
class BalanceStatusView(APIView):
    ...
    def get(self, request):
        ...
        # Barcha customerlarning umumiy balansini hisoblash
        from data.customer.models import Customer

        total_user_debt = 0
        total_customer_debt = 0
        total_user_product_debt = 0
        total_customer_product_debt = 0
        total_user_service_debt = 0
        total_customer_service_debt = 0

        # Har bir customer uchun balanslarni hisoblaymiz
        for customer in Customer.objects.all():
            # Umumiy balans
            total_balance = customer.balances.filter(**balance_filter).aggregate(
                total=Sum("change"))["total"] or 0

            # Mahsulot balansi
            product_balance = customer.balances.filter(
                payment_choice="PRODUCT", **balance_filter
            ).aggregate(total=Sum("change"))["total"] or 0

            # Xizmat balansi
            service_balance = customer.balances.filter(
                payment_choice="SERVICE", **balance_filter
            ).aggregate(total=Sum("change"))["total"] or 0

            # Qarzlarni qo'shamiz
            # Agar balance > 0: bu bizning mijozga qarzimiz (user_debt)
            if total_balance > 0:
                total_user_debt += total_balance
            # Agar balance < 0: bu mijozning bizga qarzi (customer_debt)
            elif total_balance < 0:
                total_customer_debt += abs(total_balance)

            if product_balance > 0:
                total_user_product_debt += product_balance
            elif product_balance < 0:
                total_customer_product_debt += abs(product_balance)

            if service_balance > 0:
                total_user_service_debt += service_balance
            elif service_balance < 0:
                total_customer_service_debt += abs(service_balance)
        ...
        return Response(
            {
                ...
                "customer_product_debt": total_customer_product_debt,
                "customer_service_debt": total_customer_service_debt,

                # Admin qarzi
                "user_debt": total_user_debt,
                "user_product_debt": total_user_product_debt,
                "user_service_debt": total_user_service_debt,
