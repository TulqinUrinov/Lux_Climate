...
class BalanceReportExportView(APIView):
    ...
    def get(self, request):
        ...
        if not start_date or not end_date or not customer_id:
            return Response({"error": "start_date, end_date va customer_id kerak"},
        ...
        try:
            ...
            return Response({"error": "Sana yoki customer_id noto'g'ri. Sana formati: YYYY-MM-DD"},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        # Header
        headers = ['Sana', 'Turi', 'Sabab', "Boshlang'ich summa", 'O‘zgarish',
                   'Oxirgi summa', 'Izoh']
