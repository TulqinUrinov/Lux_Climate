...
def send_order_to_customer(order, created=True):
    if not order.customer:
        return

    customer_bot_users = list(order.customer.bot_user.all()) if order.customer else []
    all_user_bot_users = list(BotUser.objects.filter(user__isnull=False))
